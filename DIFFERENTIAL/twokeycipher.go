...
//	r := rand.New(rand.NewSource(99)) // TODO: replace with time?
	...
	for number := 0 ; number < max; number++ {
		...
		tup.ct1 = simple_cipher2(key, tup.pt1)
		tup.ct2 = simple_cipher2(key, tup.pt2)
...
func simple_cipher2(key uint16, plaintext byte) byte {
	key1 := byte(key >> 8)
	key2 := byte(key & 0xFF)
	return sbox0[plaintext ^ key1] ^ key2
}
